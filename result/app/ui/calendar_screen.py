...
class CalendarScreen(tk.Frame):
    ...
    def _render_calendar(self):
        ...
        # Day-of-week header
        dow = tk.Frame(self._cal_frame, bg=CAL_BG)
        dow.pack()
        for i, d in enumerate(KOR_DAYS):
            color = CAL_WEEKEND if i in (0, 6) else CAL_MUTED
            tk.Label(dow, text=d, bg=CAL_BG, fg=color,
                     font=F(10), width=4, anchor="center"
                     ).grid(row=0, column=i, padx=CELL_PX, pady=(0, 8))

        # Date grid — 항상 6행 렌더링
        grid = tk.Frame(self._cal_frame, bg=CAL_BG)
        grid.pack()
